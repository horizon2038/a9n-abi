...
pub fn configure(
    ...
    unsafe {
        ...
        in("x2")    a2,       // info
        in("x3")     a3,       // address_space_descriptor
        in("x4")     a4,       // root_node_descriptor
        in("x5")    a5,       // frame_ipc_buffer_descriptor
        in("x6")    a6,       // notification_port
        in("x7")    a7,       // ipc_port_resolver
        in("x9")    a8,       // instruction_pointer
        in("x10")    a9,       // stack_pointer
        options(nostack),
        );
    }
...
pub fn read_register(descriptor: CapabilityDescriptor, count: Word) -> CapabilityResult {
    ...
    unsafe {
        ...
        inout("x1") a1 => a1, // oepration  -> capablity_error
        in("x2")    a2,       // count
        out("x3")     a3,       // register 3
        out("x4")     a4,       // register 4
        out("x5")    a5,       // register 5
        out("x6")    a6,       // register 6
        out("x7")    a7,       // register 7
        out("x9")    a8,       // register 8
        out("x10")    a9,       // register 9
        options(nostack),
        );
    }
